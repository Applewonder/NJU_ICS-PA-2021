...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0)
	{
		...
		if (exp >= 0xff)
		{
			...
			sig_grs = sig_grs & 0xfffffffffc000000;
			exp = 0xff;
			overflow = true;
		}
		if (exp == 0)
	...
	if (!overflow)
	{
		...
		uint64_t tmp = sig_grs & 0x0000000000000007;
		if (tmp < 4) {
		    sig_grs >>= 3;
		} else if(tmp > 4) {
		    sig_grs >>= 3;
	        ...
	        if ((sig_grs & 0x0000000000000008) == 8) {
	            sig_grs >>= 3;
	            sig_grs++;
	        } else {
	            sig_grs >>= 3;
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	FLOAT res;
	res.val = internal_normalize(f.sign, exp_res, sig_res);
	if(tes != res.fval) {
	    printf("tes = %f, res = %f \n", tes, res.fval);
	    printf("fa = %f, fb = %f\n", fa.fval, fb.fval);
	    printf("a = %d, b = %d\n", a, b);
	    printf("f.sign = %d, exp_res = %d, sig.res = %d\n", f.sign, exp_res, sig_res);
	    printf("fa.sign = %d, fa.exponent = %d, fa.fraction = %d\n", fa.sign, fa.exponent, fa.fraction);
